...
import static org.apache.dolphinscheduler.api.enums.Status.QUERY_PROCESS_DEFINITION_GLOBAL_PARAMS_ERROR;
...
public class ProcessDefinitionController extends BaseController {
     ...
     * @return process definition detail
     */
    @ApiOperation(value = "queryGlobalParamsByDefinitionId", notes = "QUERY_GLOBAL_PARAMS_BY_DEFINITION_ID_NOTES")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "processDefinitionId", value = "PROCESS_DEFINITION_ID", required = true, dataType = "Int", example = "100")
    })
    @GetMapping(value = "/global-params")
    @ResponseStatus(HttpStatus.OK)
    @ApiException(QUERY_PROCESS_DEFINITION_GLOBAL_PARAMS_ERROR)
    public Result queryGlobalParamsByDefinitionId(@ApiIgnore @RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                                  @ApiParam(name = "projectName", value = "PROJECT_NAME", required = true) @PathVariable String projectName,
                                                  @RequestParam("processDefinitionId") Integer processDefinitionId
    ) {
        logger.info("query detail of process definition, login user:{}, project name:{}, process definition id:{}",
                loginUser.getUserName(), projectName, processDefinitionId);
        Map<String, Object> result = processDefinitionService.queryGlobalParamsByDefinitionId(processDefinitionId);
        return returnDataList(result);
    }
